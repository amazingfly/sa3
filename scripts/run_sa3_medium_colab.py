...
def run(
    ...
    output_root = Path(str(first_source.get("output_dir") or os.environ.get("SA3_OUTPUTS_DIR") or ROOT / "outputs"))
    if not output_root.is_absolute():
        output_root = ROOT / output_root
    library_root = Path(
        str(first_source.get("music_library_dir") or os.environ.get("SA3_LIBRARY_DIR") or ROOT / "musicLibrary")
    )
